...
class Queue{
    ...
    void enqueue(T element){
        ...
        if(size == capacity){
           ...
           int j=0;
           //first we traverse from first index till the very end and copy every eleemner
           for (int i =first_index ; i<capacity ;i++){
               new_Data[j]=data[i];
               j++;
           }
           //we traversre from index 0 to first index , to copy the elemendt we added in the front
